...
const reviewService = {
  ...
  getReviewsByProduct(productId) {
    return axios.get(`${BASE_URL}/product/${productId}`)
      .then(response => {
        console.log(`Fetched Reviews for Product ID ${productId}:`, response.data); // Log reviews for the product
        return response;
      })
      .catch(error => {
  ...
  deleteReview(id) {
    return axios.delete(`${BASE_URL}/${id}`)
      ...
      .catch(error => {
        console.error(`Error deleting review with ID ${id}:`, error.response || error.message || error);
        throw error;
      });
  }
};
