...
const getPresignedUrl = async (fileName, contentType) => {
  console.log(`Requesting presigned URL for file: ${fileName} with content type: ${contentType}`);
  ...
  try {
    ...
    if (!response.data || !response.data.url) {
      console.error('Presigned URL response does not contain "url" property:', response.data);
...
export const uploadFileToS3 = async (file) => {
  ...
  try {
    ...
    if (uploadResponse.status !== 200) {
      console.error(`Failed to upload file to S3. Status code: ${uploadResponse.status}`);
