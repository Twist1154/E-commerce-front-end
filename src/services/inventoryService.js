...
const inventoryService = {
  ...
  createInventoryItem(inventoryItem) {
    return axios
      .post(`${BASE_URL}/create`, inventoryItem)
      .then((response) => {
  ...
  getInventoryItemById(id) {
    return axios
      ...
      .catch((error) => {
        console.error(
          `Error fetching inventory item with ID ${id}:`,
          error.response?.data || error.message,
        ); // Log error
        throw error;
      });
  ...
  updateInventoryItem(inventoryItem) {
    return axios
      .put(`${BASE_URL}/update`, inventoryItem)
      .then((response) => {
  ...
  deleteInventoryItem(id) {
    return axios
      ...
      .then((response) => {
        console.log(
          `Inventory item with ID ${id} deleted successfully:`,
          response.data,
        ); // Log success
        return response.data; // Return only the data
      })
      .catch((error) => {
        console.error(
          `Error deleting inventory item with ID ${id}:`,
          error.response?.data || error.message,
        ); // Log error
        throw error;
      });
  ...
  getInventoryItemsByProductId(productId) {
    return axios
      ...
      .then((response) => {
        console.log(
          `Fetched inventory items for product ID ${productId}:`,
          response.data,
        ); // Log success
        return response.data; // Return only the data
      })
      .catch((error) => {
        console.error(
          `Error fetching inventory items with product ID ${productId}:`,
          error.response?.data || error.message,
        ); // Log error
        throw error;
      });
